...
const Table = ({ searchAction }) => {
    ...
    const duplicateAction = (id) => {
        ...
        const notificationToCopy = notifications.find(({ id: _id }) => id === _id);
        const copiedNotification = { ...notificationToCopy };
        const newId = notifications.length + 1;
        copiedNotification.id = newId;
        copiedNotification.title = copiedNotification.title + " - Copy";
        setNotifications([...notifications, copiedNotification]);
    };

    const deleteAction = id => {
        setNotifications(notifications.filter(({ id: _id }) => id !== _id))
    }
    ...
    const currentItems = defaultNotifications.slice(indexOfFirstItem, indexOfLastItem);
    ...
    return (
        <>
                    ...
                    <tr>
                        <th scope="col"><input type="checkbox" checked={selectedNotifications.length === notifications.length} onChange={() => {
                            if (selectedNotifications.length === notifications.length) {
                                ...
                                setSelectedNotifications(notifications.map(({ id }) => id))
                            }
                        }} /></th>
                ...
                <tbody>
                    {
                        notifications.map(notification => {
                            ...
                            return <tr key={id}>
                                <th scope="row">
                                    <input type="checkbox" checked={selectedNotifications.includes(id)} onChange={() => {
                                        setSelectedNotifications(
                                ...
                                <td>{title}</td>
                                <td> <span className="bg-blue-300 px-4 py-1 rounded-3xl ">{type} </span></td>
                                <td>{stats}</td>
                                <td>
                                    <input type="checkbox" checked={'active' === status} onChange={() => {
                                        setNotifications(notifications.map((_notification) => {
                                            const { id: _id, status } = _notification
                                            if (_id === id) {
                                                _notification.status = 'active' === status ? 'inactive' : 'active'
                                            }
            ...
            <Pagination
                ...
                totalItems={defaultNotifications.length}
